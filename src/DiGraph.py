...
class DiGraph(GraphInterface):
    ...
    def __repr__(self):
        return "Graph: |V|=%s , |E|=%s" %(self.v_size(),self.edges_size)
    ...
    def remove_edge(self, node_id1: int, node_id2: int) -> bool:
       if node_id1 in self.nodes and node_id2 in self.nodes:
           if node_id1 in self.edges_out:
               if node_id2 in self.edges_out[node_id1]:
                   ...
                   self.nodes[node_id1].edges_out.pop(node_id2)
                   self.nodes[node_id2].edges_in.pop(node_id1)
